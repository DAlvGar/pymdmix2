...
# Mapping from water model name to its LEaP leaprc file.  These are sourced
# automatically when ion addition is requested so that Na+ / Cl- etc. are
# available as named units inside tleap.
_WATER_MODEL_LEAPRC: dict[str, str] = {
    "TIP3P": "leaprc.water.tip3p",
    "TIP4P": "leaprc.water.tip4pew",
    "SPC": "leaprc.water.spcew",
...
def generate_leap_script(
    ...
    * **Amber Object File** (``*.off``, ``*.lib``): the primary/recommended
    ...
    # When ions will be added, ensure the water/ion leaprc is sourced so that
    # Na+, Cl- etc. are available as named units inside tleap.
    if ion_config.neutralize or (ion_config.add_ions and ion_config.concentration > 0):
        water_leaprc = _WATER_MODEL_LEAPRC.get(
            solvent.water_model.upper(),
            "leaprc.water.tip3p",  # fall back to TIP3P
        )
        if water_leaprc not in force_fields:
            force_fields = list(force_fields) + [water_leaprc]
...
def run_leap(
    ...
    try:
        ...
        # Detect real LEaP failures.  We must be precise here because
        # "Errors = 0" and "Warnings = N" appear in the normal summary line
        # "Exiting LEaP: Errors = 0; Warnings = N; Notes = 0."
        # so a simple substring search for "Error" would give false positives.
        #
