...
class MDSettingsConfigFileParser:
    ...
    def _split_per_replica_slash(
        self,
        string: str | None,
        val_control: list[str] | None = None,
        val_format: type = str
    ...
    def _split_per_replica_colon(
        ...
        for solv in self.solvents:
            if solv in temp:
                ...
                raise MDSettingsParserError(
                    f"No value for solvent {solv} and no common value"
                )
    ...
    def parse(
        ...
        if not md_sections:
            raise MDSettingsParserError(
                "No sections starting with 'MDSETTINGS' found."
            )
        ...
        for section in md_sections:
            ...
            if not solvents_str:
                raise MDSettingsParserError(
                    "SOLVENT(S) option missing in config file."
                )
            ...
            restr_split = self._split_per_replica_slash(
                restr, val_control=['BB', 'HA', 'FREE']
            )
            ...
            main_opts = {
                'restr', 'solvents', 'solvent', 'nanos', 'temp', 'force',
                'nrepl', 'restrmask', 'alignmask'
            }
            ...
            if md_settings_keys:
                for k, v in section_dict.items():
                    ...
                    if matches:
                        ...
                        raise MDSettingsParserError(
                            f"Unknown attribute '{k}' in MD settings"
                        )
...
class SystemConfig:
    ...
    extra_residues: Optional[List[str]] = None
    extra_forcefields: Optional[List[str]] = None
...
class ReplicaConfig:
    ...
    align_mask: Optional[str] = None
    md_settings: Optional[Path] = None
