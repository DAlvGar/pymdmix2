...
class DCDReader:
    ...
    def _read_header(self) -> None:
        ...
        if not (header_data[0] == 84 and 
                header_data[1] == b'CORD' and 
                header_data[-1] == 84 and 
                header_data[13] != 0):
            raise DCDFormatError(
                "Invalid DCD format. Expected 32-bit big-endian CORD format."
            )
    ...
    def read_all(self) -> NDArray[np.float32]:
        ...
        all_frames = np.zeros(
            (self._header.n_frames, self._header.n_atoms, 3),
            dtype=">f4"
        )
    ...
    def get_frame(self, index: int) -> DCDFrame:
        ...
        if index < 0 or index >= self._header.n_frames:
            raise IndexError(
                f"Frame index {index} out of range [0, {self._header.n_frames})"
            )
