...
| [AmberTools](#3-ambertools--cpptraj-tests) | `ambertools` / `cpptraj` | AmberTools installation (tleap, cpptraj) | ~40 |
...
### AmberTools (for `ambertools` / `cpptraj` tier)

Install AmberTools from [ambermd.org](https://ambermd.org) or via conda-forge:
...
## 3. AmberTools / cpptraj tests

### 3a. AmberTools tests (`ambertools`)

Require **tleap** (LEaP). These tests check the full solvation pipeline and
...
### 3b. cpptraj tests (`cpptraj`)

Require **cpptraj**. Tests run density calculations using the cpptraj binary.

```bash
uv run pytest -m cpptraj -v tests/
```

### 3c. Combined: cpptraj + real trajectory data (`cpptraj and real_data`)

These tests run cpptraj against the real 20-frame `traj.nc` trajectory. They
are the most comprehensive density validation and require **both** cpptraj
*and* the Git LFS data.
...
The Dockerfile bundles AmberTools + the full pymdmix package. Use it to run
AmberTools and real-data tests without modifying your local environment.
...
### AmberTools tests
1. Add `@pytest.mark.ambertools` (and `@pytest.mark.cpptraj` where cpptraj is
   needed) to the test class.
2. Use the `solvated_pep` session fixture from `test_ambertools_e2e.py` if you
   need a solvated topology generated by tleap.
3. If your test needs both cpptraj and real data, add both markers:
